...
interface ColumnDropdownProps {
  ...
  t: (key: string) => string;
  /** Translation keys for the dropdown labels */
  translationKeys?: {
    columns?: string;
    showColumns?: string;
  };
...
/**
 * Generic column visibility and ordering dropdown component
 * Can be used across different pages (Orders, Expenses, Transfers, etc.)
 */
export function ColumnDropdown({
  ...
  const columnsLabel = translationKeys.columns || "common.columns";
  const showColumnsLabel = translationKeys.showColumns || "common.showColumns";
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        aria-label={t(columnsLabel) || "Columns"}
      >
        <svg
          className="w-5 h-5"
          fill="none"
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-slate-200 z-50 py-2">
          <div className="px-4 py-2 text-xs font-semibold text-slate-500 uppercase border-b border-slate-200">
            {t(showColumnsLabel) || "Show Columns"}
          </div>
